...
const ROLE_META = {
  school_student: { label: 'Maktab o\'quvchisi', Icon: GraduationCap, color: '#2563EB', grad: 'linear-gradient(135deg, #2563EB 0%, #7C3AED 100%)' },
  student:        { label: 'Talaba',              Icon: Student,          color: '#7C3AED', grad: 'linear-gradient(135deg, #7C3AED 0%, #A855F7 100%)' },
  teacher:        { label: 'O\'qituvchi',         Icon: ChalkboardTeacher,color: '#16A34A', grad: 'linear-gradient(135deg, #16A34A 0%, #22C55E 100%)' },
};
    ...
    <div className="space-y-5 pb-6">
      ...
      <div className="rounded-2xl overflow-hidden border border-[var(--border)] bg-[var(--surface)]">
        ...
        <div className="px-5 pb-5">
          ...
          <div className="relative -mt-10 mb-3 w-fit">
            <div className="w-20 h-20 rounded-2xl border-4 border-[var(--surface)] flex items-center justify-center text-white shadow-lg"
              style={{ background: roleMeta.grad }}>
          ...
          <div className="mt-3">
            ...
            <div className="h-2 rounded-full bg-[var(--background)] overflow-hidden">
              <div className="h-full rounded-full transition-all"
                style={{ width: `${xpPercent}%`, background: roleMeta.grad }} />
            </div>
              ...
              <div className="space-y-1.5">
                ...
                <div className="h-1.5 rounded-full" style={{ background: darkMode ? 'rgba(255,255,255,0.1)' : 'rgba(0,0,0,0.08)' }}>
                  <div className="h-full rounded-full transition-all" style={{
                    width: `${pct}%`,
                    background: 'linear-gradient(90deg, #CD7F32, #D4A820, #7C3AED, #EF4444)',
                  }} />
                </div>
                {/* tier dots */}
      ...
      <div className="rounded-2xl border border-[var(--border)] bg-[var(--surface)] overflow-hidden divide-y divide-[var(--border)]">
        ...
        <div className="px-4 py-3.5 flex items-center gap-3">
          <div className="w-9 h-9 rounded-xl flex items-center justify-center shrink-0" style={{ background: '#EF444418' }}>
            <Bell size={18} style={{ color: '#EF4444' }} />
          </div>
          <div className="flex-1">
            <p className="text-sm font-medium text-[var(--text-primary)]">Bildirishnomalar</p>
            <p className="text-[11px] text-[var(--text-secondary)]">{notifEnabled ? 'Yoqilgan' : "O'chirilgan"}</p>
          </div>
          <button onClick={() => setNotifEnabled(v => !v)}
            className={cn('w-12 h-6 rounded-full transition-all relative shrink-0', notifEnabled ? 'bg-red-400' : 'bg-[var(--border)]')}>
            <div className={cn('w-5 h-5 rounded-full bg-white absolute top-0.5 transition-all shadow-sm', notifEnabled ? 'left-6' : 'left-0.5')} />
          </button>
        </div>
