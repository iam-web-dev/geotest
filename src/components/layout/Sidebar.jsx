    ...
    <aside className="hidden lg:flex flex-col w-64 h-screen sticky top-0 bg-[var(--surface)] border-r border-[var(--border)] p-4">
      ...
      <div className="border-t border-[var(--border)] pt-4 mt-4">
        <NavLink to="/profile" className="flex items-center gap-3 px-3 py-2.5 rounded-[var(--radius-sm)] hover:bg-[var(--background)] transition-all duration-200">
          <div className="w-9 h-9 rounded-full flex items-center justify-center text-white shrink-0 overflow-hidden"
            style={{ background: 'linear-gradient(135deg, var(--primary) 0%, var(--primary-hover) 100%)' }}>
            {merged.avatar
              ? <img src={merged.avatar} alt={merged.name} className="w-full h-full object-cover" />
